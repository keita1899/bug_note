...
export default function BugFormContainer() {
  ...
  const { data, isPending } = useQuery<Bug>({
    queryKey: ['bug', id],
  ...
  const mutation = useMutation({
    mutationFn: async (data: BugFormValues) => {
      try {
        const res = await axios.patch(API_URLS.BUG.UPDATE(String(id)), data, {
    ...
    <Layout>
      {isPending ? (
        ...
        >
          ...
          >
            <Input name="error_message" register={register} />
          </FormField>
